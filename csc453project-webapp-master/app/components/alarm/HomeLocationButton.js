...
import { selectButton, setHomeLocationText } from '../../reducers/alarm/alarmActions';

class HomeLocationButton extends Component {
    ...
    buttonPressed(set) {
        this.props.selectButton(set ? "location" : "");
    }
    ...
    submitLocation() {
        ...
        axios.post('/api/alarm/homelocation', {
            location: this.state.value
        }).then(res => {
            if (res.status == 200) {
                this.props.setHomeLocationText(res.data.location);
...
function mapStateToProps(state) {
    return {
        ...
        locationText: state.alarm.homeLocationText
    };
}
...
function matchDispatchToProps(dispatch) {
    return bindActionCreators({
        ...
        setHomeLocationText: setHomeLocationText
    }, dispatch);
}

export default connect(mapStateToProps, matchDispatchToProps)(HomeLocationButton);
